...
SolvedFormSubproblemDisjunction::SolvedFormSubproblemDisjunction(int nrBindings)
...
SolvedFormSubproblemDisjunction::~SolvedFormSubproblemDisjunction()
{
...
void
SolvedFormSubproblemDisjunction::markReachableNodes()
{
...
void
SolvedFormSubproblemDisjunction::addOption(LocalBinding* difference,
...
bool
SolvedFormSubproblemDisjunction::unificationSolve(bool findFirst, UnificationContext& solution)
{
  int nrOptions = options.length();
  if (findFirst)
    selectedOption = 0;
  for (; selectedOption < nrOptions; selectedOption++)
      ...
      if (findFirst && so.difference != 0)
	{
	  ...
	  if (!(so.difference->solvedFormAssert(solution, assertionSubproblem)))
	    goto fail;
	  if (assertionSubproblem != 0 && !(assertionSubproblem->unificationSolve(true, solution)))
	    goto fail;
	}
      for (Subproblem* subproblem = so.subproblem;; findFirst = true)
	{
	  if (subproblem == 0)
	    ...
	    {
	      if (subproblem->unificationSolve(findFirst, solution))
		return true;
	    }
	  //
	  ...
	  //	it in a new way.
	  //
	  if (assertionSubproblem == 0 || !(assertionSubproblem->unificationSolve(false, solution)))
	    break;
	}

...
{
  FOR_EACH_CONST(i, RetainedUnifierMultimap, mostGeneralSoFar)
    delete i->second;
}
...
UnifierFilter::markReachableNodes()
{
  FOR_EACH_CONST(i, RetainedUnifierMultimap, mostGeneralSoFar)
    {
      const Substitution& unifier = i->second->unifier;
      int nrFragile = unifier.nrFragileBindings();
      for (int i = 0; i < nrFragile; i++)
	{
	  DagNode* d = unifier.value(i);
	  if (d != 0)
	    d->mark();
	}
    }
...
void
UnifierFilter::insertUnifier(const Substitution& unifier, int positionIndex, int equationIndex /*, int nrFreeVariables */)
{
  //
  ...
  //
  RetainedUnifierMultimap::iterator i = mostGeneralSoFar.begin();
  while (i != mostGeneralSoFar.end())
    {
      RetainedUnifierMultimap::iterator next = i;
      ++next;

      RetainedUnifier* potentialVictim = i->second;
  ...
  //newUnifier->nrFreeVariables = nrFreeVariables;
  mostGeneralSoFar.insert(RetainedUnifierMultimap::value_type(positionIndex, newUnifier));
}

bool
UnifierFilter::getNextSurvivingUnifier(Substitution*& unifier, int& positionIndex, int& equationIndex /*, int& nrFreeVariables */)
{
  ...
  RetainedUnifier* r = nextUnifier->second;
  unifier = &(r->unifier);
  positionIndex = nextUnifier->first;
  equationIndex = r->equationIndex;
  // nrFreeVariables = r->nrFreeVariables;
  return true;
}
  ...
  for (int i = 0; i < nrInterestingVariables; ++i)
    {
      ...
      t = t->normalize(true);  // needed? or will unifier always be in normal form?

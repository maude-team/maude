			 ...
			 bool rangeKnown)
{
  ...
  s << '(';
  prettyPrint(s, st->getArgument(), PREFIX_GATHER, UNBOUNDED, 0, UNBOUNDED, 0, rangeKnown);
  s << ')';
  return true;
}
			  ...
			  bool rangeKnown)
{
  ...
  //
  if (handleIter(s, term, si, rangeKnown))
    return;
  ...
  switch (basicType)
    ...
    case SymbolType::MINUS_SYMBOL:
      {
	if (handleMinus(s, term, rangeKnown))
	  return;
	break;
      }
    case SymbolType::DIVISION_SYMBOL:
      {
	if (handleDivision(s, term, rangeKnown))
	  return;
	break;
      }
    case SymbolType::FLOAT:
      {
	handleFloat(s, term, rangeKnown);
	return;
      }
    case SymbolType::STRING:
      {
	handleString(s, term, rangeKnown);
	return;
      }
    case SymbolType::QUOTED_IDENTIFIER:
      {
	handleQuotedIdentifier(s, term, rangeKnown);
	return;
      }
    case SymbolType::VARIABLE:
      {
	handleVariable(s, term, rangeKnown);
	return;
      }
    default:
      break;
  ...
  if ((printMixfix && si.mixfixSyntax.length() != 0) ||
      ...
      for (int arg = 0; moreArgs; arg++)
	{
      ...
      printTails(s, si, pos, nrTails, needAssocParen, true);
      if (UserLevelRewritingContext::interrupted())
	return;
      if (needParen)
	s << ')';
